...
import { BehaviorSubject, combineLatest, concatAll, concatMap, debounceTime, delay, exhaustMap, filter, from, map, merge, mergeMap, Observable, of, shareReplay, Subject, tap, toArray, withLatestFrom, zip } from 'rxjs';
...
export class ExhaustMapService {
  ...
  filteredCustomers$: Observable<Customer[]> = combineLatest([
    ...
    concatMap(([crud, filterOrReset]: [Customer[], HOMFilters]) => this.getFilteredCustomersFromBackend$(crud, filterOrReset)),
    log('filteredCustomers$', undefined, 'orange')
  )
