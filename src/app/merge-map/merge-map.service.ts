...
import { BehaviorSubject, combineLatest, concatAll, concatMap, debounceTime, distinctUntilChanged, filter, first, from, groupBy, map, merge, mergeAll, mergeMap, Observable, of, reduce, share, shareReplay, Subject, take, tap, throttleTime, toArray, withLatestFrom, zip } from 'rxjs';
...
export class MergeMapService {
  ...
  filteredCustomersByJurisdiction$: Observable<JurisdictionCustomers[]> = combineLatest([
    ...
    sortObs('last'),
    mergeMap((customers: Customer[]) => this.groupCustomersByJurisdiction$(customers)),
  ...
  groupCustomersByJurisdiction$(customers: Customer[]): Observable<JurisdictionCustomers[]> {
     return from(customers).pipe(
        ...
        mergeMap(([jurisdictionCode, customers]) => this.apiService.getJurisdictionByCode$(jurisdictionCode)
